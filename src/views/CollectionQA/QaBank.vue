  ...
  <div class="com-container">
    ...
    <template v-else>
      <view-box minus="40px" class="classify ">
        ...
        <p>关联文章:</p>
        <p>文章来源:</p>
        <div class="btn">问答反馈</div>
      </view-box>
      <div class="bottom-btn" @click="changestate" :class="{ disabled: status === 'loading' }">{{ is_collected ? '取消' : '添加' }}收藏</div>
    </template>
  </div>
</template>
...
export default {
  ...
  activated() {
    if (this.$route.query.qaId) {
      this.question_id = this.$route.query.qaId
    }
    this.load()
